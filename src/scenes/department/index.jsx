...
const Department = () => {
  ...
  const fetchIssuingAuthority = async () => {
    try {
      ...
      console.error("Lỗi tải danh sách cơ quan ban hành:", error);
    }
  };

  const fetchData = useCallback(async () => {
    try {
      const res = await api.get(`/v1/department`, {
        params: {
  ...
  const handleCreateSubmit = (values) => {
    api
      .post("/v1/department", {
        name: values.name,
        issuingauthority_id: values.issuing_authority.uuid,
      })
      .then((response) => {
        fetchData();
  ...
  const handleDeleteConfirm = () => {
    api
      .delete(`/v1/department/${selectedRow.uuid}`)
      .then((response) => {
        fetchData();
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: "Thuộc cơ quan ban hành",
      flex: 1,
  ...
  return (
    <Box m="20px">
      <Header
        ...
        subtitle="Quản lý thông tin phòng ban trong hệ thống"
      />
      <Box
        display="flex"
